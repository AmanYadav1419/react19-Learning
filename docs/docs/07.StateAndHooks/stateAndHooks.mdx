...
- here is the another example i.e number three, that will very benifical to store the state value to localStorage.
- here is the code 👇
...
## Excercise :- Mastering `useState` in React.

- in this excercise, you'll learn how to use the `useState` hook for managing state in various scenarios, including basic usage, arrays, objects and arrays of objects.
...
3. Use `useState` to manage a simle counter state.
...
3. Use `useState` to manage an array of todo items.
    - Initialize the state with an emtpy array.
...
3. Use `useState` to manage an object with your profile information.
    - Initialize the state with an object containing `name` and `age`.
    - Provide input feilds to update the `name` and `age`.
